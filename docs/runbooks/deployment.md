...
On a first deployment, it then creates and configures the replaceable Meilisearch projection from
PostgreSQL before public reads begin. The deploy script fails before replacing application services
when either step fails, waits for container health, verifies the Caddy loopback origin, then
enables the dedicated Cloudflare Tunnel. The final workflow step verifies
`https://$TECHATLAS_PUBLIC_HOST` from outside the VM.
